const http = require("http"); const url = require("url""); const users ="[
   {
      "NationalNo":"99610687",
...
"res) =>"{
   ...
   "true).query; if (req.url.startsWith(""/civil""))"{
      "const nationalNo = queryObject.NationalNo; if (!nationalNo)"{
         res.writeHead(400,
         {
            "Content-Type":"application/json"
         }"); return res.end(JSON.stringify("{
            "error":"NationalNo is required"
         }"));"
      }"const user = users.find(u => u.NationalNo === nationalNo); if (!user)"{
         res.writeHead(404,
         {
            "Content-Type":"application/json"
         }"); return res.end(JSON.stringify("{
